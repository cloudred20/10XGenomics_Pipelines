...
Note: The input data — 10k_pbmc_ATACv2_nextgem_Chromium_Controller (filtered_peak_bc_matrix.h5, fragments.tsv, singlecell.csv) — is excluded from version control due to file size limits. The raw files can be downloaded directly from the [Signac PBMC Vignette](https://stuartlab.org/signac/articles/pbmc_vignette#non-linear-dimension-reduction-and-clustering).
